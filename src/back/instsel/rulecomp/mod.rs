...
pub fn compile_rules(input: &str, filename: &str) -> String {
    ...
    format!("// AUTOGENERATED CODE! DO NOT EDIT!
use driver::interner::Ident;
use middle::ir;
...
fn translate_asm_arg(arg: &AsmArg, types: &HashMap<Ident, IrArg>) -> String {
    match *arg {
        ...
        AsmArg::Indirect { ref size, ref base, ref index, ref disp } => {
            format!("asm::Argument::Indirect {{ size: {}, base: {}, index: {}, disp: {} }}",
                    translate_option(size.map(|s| format!("asm::OperandSize::{:?}", s))),
                    translate_option(base.clone().map(|b| translate_asm_arg_register(&b))),
                    translate_option(index.clone().map(|(i, f)| format!("({}, {})", translate_asm_arg_register(&i), f))),
